...
clear
DataDir = '/Users/jiaxintu/Documents/Data/pacman_wrapper/TrackingTheSameNeuron/sorted_files';
% DataDir = '/Volumes/My Passport/sorted/TrackingSameNeurons/New folder';
n_channels_per_array = 32;
n_arrays = 3; % A,B,C
fileNames = dir([DataDir,'/*Pacman*']);
...
survived_neurons = cell(length(allcomb),2);
...
for irun = 1:length(allcomb)
    ...
    for arrayID =['A','B','C']
        j = double(arrayID)-'A'+1;
        array_offset = n_channels_per_array*(j-1);
        fileNames = dir([DataDir,'/*Pacman_',arrayID,'*']);
